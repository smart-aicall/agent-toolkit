...
export function registerLeadTools(server: McpServer, client: SmartCallClient) {
  server.registerTool(
    'push_lead',
    {
      description: 'Push a phone number to an AI outbound calling program. The lead will be queued and called automatically by the AI bot. Returns a threadId you can use to query the call result later.',
      inputSchema: {
  ...
  server.registerTool(
    'batch_query_leads',
    {
      description: 'Query status and call results for multiple leads at once. Filter by threadIds, programId, or date. More efficient than calling query_lead repeatedly.',
      inputSchema: {
